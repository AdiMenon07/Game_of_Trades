...
if st.session_state.team is None:
    ...
    if st.button("Continue") and team_input.strip():
...
if is_admin:
    with st.expander("⚙️ Organizer Controls (Admin Only)"):
        ...
        if st.button("♻️ Reset Round"):
            ...
            st.warning("Round reset. Start again.")
...
# ---------- PORTFOLIO ----------
portfolio_placeholder.subheader("💼 Portfolio")
if portfolio:
    portfolio_placeholder.metric("Available Cash", f"₹{portfolio['cash']:.2f}")
    if portfolio.get("holdings"):
        holdings_df = pd.DataFrame.from_dict(portfolio["holdings"], orient="index")
        portfolio_placeholder.dataframe(holdings_df, use_container_width=True)
    else:
        portfolio_placeholder.info("No holdings yet! Buy some stocks.")
...
if stocks:
    ...
    stocks_placeholder.plotly_chart(fig3d, use_container_width=True)
else:
    stocks_placeholder.warning("No stock data available.")

# ---------- LEADERBOARD ----------
leaderboard_placeholder.subheader("🏆 Live Leaderboard")
if leaderboard:
    ...
    leaderboard_placeholder.dataframe(ldf.style.apply(highlight_top3, axis=1), use_container_width=True, hide_index=False)
else:
    leaderboard_placeholder.info("No teams yet. Waiting for participants to trade...")

# ---------- NEWS ----------
news_placeholder.subheader("📰 Market News")
if news and "articles" in news and news["articles"]:
    ...
    news_placeholder.info("No news available right now.")
